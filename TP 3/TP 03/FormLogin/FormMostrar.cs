...
namespace Formularios
{
    public partial class FormMostrar : Form
    {
        ...
        private void FormatearDataGrid()
        {
            ...
            switch (mostrarInfo)
            {
                case MostrarInfo.Empleado:
                    ...
                    dgvMostrar.Columns["NombreUsuario"].DisplayIndex = 3;
                    dgvMostrar.Columns["Permisos"].DisplayIndex = 4;
                    dgvMostrar.Columns["NombreCompleto"].Visible = false;
                    break;
                case MostrarInfo.Menu:
                    dgvMostrar.DataSource = ControladorMostrar.Menu;
                    dgvMostrar.Columns["Id"].DisplayIndex = 0;
                    dgvMostrar.Columns["Nombre"].DisplayIndex = 1;
        ...
        private void RellenarStock(double cantidad)
        {
            DataGridViewRow fila = SeleccionarFila();
            if (fila is not null)
            {

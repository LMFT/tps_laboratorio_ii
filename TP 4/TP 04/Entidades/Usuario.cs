...
namespace Entidades
{
    ...
    public class Usuario : Persona, ICargar<Usuario>
    {
        ...
        private Usuario(object[] valores)
        {
            nombre = valores[0].ToString();
            apellido = valores[1].ToString();
            dni = (int)valores[2];
            nombreUsuario = valores[3].ToString();
            password = valores[4].ToString();
            permisos = (Permisos)valores[5];
        }
        ...
        public Usuario Cargar(object[] valores)
        {
            if(valores is not null)
            {
                return new Usuario(valores);
            }
            return null;
        }
